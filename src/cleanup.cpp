...
int main(int argc, char* argv[])
{
    ...
    desc.add_options()
        ...
        ("path,p", po::value<std::vector<std::string>>(),
         "Specify the path at which the recursive search begins. By default "
         "the environnement variable 'HOME' is being used.");
    ...
    if(vm.count("version"))
    {
        std::cout << "cleanup version " << CLEANUP_VERSION_STRING << std::endl;
        return 0;
    }
    ...
    // Search file system
    fstask::analyze_filesystem(path);
    ...
    std::cout << "cleanup: scanned " << trash_ptr->file_count() << " files "
              ...
              << std::chrono::duration<double, std::milli>(t_end - t_start)
                     .count() << " ms -> "
              << (vm.count("cache") ? "found " : "deleted ") << cnt
              << (cnt == 1 ? " file" : " files") << std::endl;
