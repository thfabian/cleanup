...
void fstask::analyze_filesystem(bf::path root)
{
    ...
    if(!bf::is_directory(root))
        exit_after_error(
            {"'", std::string(root.c_str()), "' is not a directory"});

    fstask& root_task = *new(tbb::task::allocate_root())
                            fstask(std::move(root));
...
tbb::task* fstask::execute()
{
        ...
        e == bs::errc::success && it != bf::directory_iterator(); ++it)
    {
        ...
        {
            fstask& child = *new(tbb::task::allocate_child())
                                fstask(std::move(p));
            increment_ref_count();
